...
aoc 2023, 4 do
  ...
  def p2(input) do
    ...
    card_map = for i <- 1..Map.size(card_map), reduce: card_map do
      card_map ->
        {matches, points, t_mult} = Map.get(card_map, i)
        if matches > 0 do
          for z <- i+1..i + matches, reduce: card_map do
            card_map ->
              Map.update(card_map, z, nil, fn {matches, points, mult} -> {matches, points, (mult + t_mult)} end)
          end
        else
          card_map
        end
