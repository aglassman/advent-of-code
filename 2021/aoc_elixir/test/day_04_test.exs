      ...
      winners = [win | _] ->
        if wins do
          ...
          |> Enum.map(fn winner ->
            ...
            win = {called_numbers, to_call, winning_numbers, winning_board} |> IO.inspect(charlists: :as_lists)
          end)

          winning_boards = Enum.map(winners, fn {_, _, board} -> board end)

          # remove board that has won
          remaining_boards = Enum.reject(boards, fn board -> Enum.any?(winning_boards, fn x -> x == board end) end)
